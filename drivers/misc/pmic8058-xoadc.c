...
#if 1
void htc_sample_mpp(uint32_t adc_instance)
{
	...
	/* MPP address, EX: 0x50 for MPP01, 0x51 for MPP02	*/
	printk(KERN_INFO "sff:");
	for (mpp_num = 0; mpp_num < 0x0C; mpp_num++) {
		addr = 0x50+mpp_num;
		pm8058_read(adc_pmic->pm_chip, addr, &values, 1);
		printk(KERN_INFO "0x%x=0x%x, ", 0x50+mpp_num, values);
	}
	printk(KERN_INFO "\n");
}
#endif
