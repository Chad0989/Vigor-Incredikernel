	...
	while (rem) {
		...
		do {
			...
			if (!timeout) {
				...
				istatus = readl(dev->base + QUP_I2C_STATUS);
				qstatus = readl(dev->base + QUP_ERROR_FLAGS);
				op_flgs = readl(dev->base + QUP_OPERATIONAL);
				dev_err(dev->dev, "[QUP I2C Err] after QUP_SW_RESET, I2C Status: %x,QUP Status: %x, OP Flags: %x\n", istatus, qstatus, op_flgs);
